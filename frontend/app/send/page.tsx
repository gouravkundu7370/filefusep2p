...
export default function SendPage() {
  ...
  const connectWebSocket = () => {
    ...
    ws.onopen = () => {
      console.log("WebSocket connected from Sender 🚀");
  ...
  const copyLink = () => {
    ...
    navigator.clipboard
      .writeText(`${window.location.origin}/receive?id=${transferId}`)
      .then(() => {
        setCopied(true);
  ...
  return (
    <div className="min-h-screen flex flex-col gradient-bg">
      ...
      <main className="flex-1 container flex flex-col items-center justify-center py-8 max-w-md">
        <Card className="w-full subtle-border shadow-lg hover-lift bg-primary-foreground/80 backdrop-blur-sm overflow-hidden">
          <CardContent className="p-6">
            <div className="w-full space-y-8">
              {!file && (
                <>
                  <div className="text-center space-y-3">
                    <h1 className="text-3xl sm:text-4xl lg:text-6xl font-bold tracking-tight bg-clip-text text-transparent bg-gradient-primary leading-tight">
                      Send a File
                    </h1>
                  ...
                  >
                    <Input
                      ...
                      className="hidden"
                    />
                    <div className="w-16 h-16 mx-auto rounded-xl bg-accent/20 flex items-center justify-center mb-4 group-hover:scale-105 transition-transform">
                      <FileIcon className="h-7 w-7 text-accent" />
                    </div>
                    <p className="text-base text-muted-foreground font-bold mb-1">
                  ...
                  {isConnected ? (
                    <div className="w-full space-y-4">
                      {transferProgress > 0 ? (
                        ...
                        <Button
                          className="w-full py-6 rounded-xl hover-lift bg-accent hover:bg-accent/90 text-accent-foreground"
                          onClick={sendFile}
                    ...
                    <div className="flex items-center justify-center space-x-2 text-sm text-muted-foreground p-4 rounded-xl bg-secondary/50 glass">
                      <div className="w-2 h-2 bg-primary rounded-full animate-pulse-soft mr-2"></div>
                      <div
                        className="w-2 h-2 bg-primary rounded-full animate-pulse-soft mr-2"
